...
elseif ($_SESSION['user']->privilege() != 9) {
	$_SESSION['msg'] = new Message('Vous ne disposez pas des droits suffisants pour accéder à la page <code>'. basename(__FILE__) .'</code>.<br />Vous avez été redirigé(e) sur la page d’accueil.', -1, 'Accès refusé !');
	header('Location: /');
	exit();
}
elseif (!Activity::isActivityURL('bibliotheque')) {
